...
$estimate_invoice = $customData['estimate_invoice'];
    ...
    {{-- Header --}}
    @if ($invoice->logo)
        <img src="{{ str_starts_with($invoice->logo, 'data:') ? $invoice->logo : asset($invoice->logo) }}" alt="logo"
            height="75" class="pl-0">
    @endif
    <table class="table">
        <tbody>
            <tr>
                <td class="px-0">
                    ...
                    @if ($company_info['companyWebsite'])
                        <p class="no-margin-p">{{ get_label('website', 'Website') }}:
                            {{ $company_info['companyWebsite'] }}</p>
                    @endif
                    @if ($company_info['companyVatNumber'])
                        <p class="no-margin-p">{{ get_label('vat_number', 'VAT Number') }}:
                            {{ $company_info['companyVatNumber'] }}</p>
                    @endif
                </td>
        ...
        <tbody>
            <tr>
                ...
                <td class="border-0 pl-0">
                    ...
                    @endif
                    <p><?= $estimate_invoice->type == 'estimate' ? get_label('estimate_no', 'Estimate No.') : get_label('invoice_no', 'Invoice No.') ?>:<strong>
                            #{{ $estimate_invoice->type == 'estimate' ? get_label('estimate_id_prefix', 'ESTMT-') : get_label('invoice_id_prefix', 'INVC-') }}{{ $estimate_invoice->id }}</strong>
                    </p>
                    <p>{{ get_label('from_date', 'From date') }}:
                        <strong>{{ format_date($estimate_invoice->from_date) }}</strong></p>
                    <p>{{ get_label('to_date', 'To date') }}:
                        <strong>{{ format_date($estimate_invoice->to_date) }}</strong></p>
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <table class="table-items table">
        <thead>
            ...
            @if (count($estimate_invoice->items) > 0)
                <tr>
                    <th scope="col" class="border-0 pl-0 text-left">{{ get_label('serial_no', 'S.No') }}</th>
                    <th scope="col" class="border-0 pl-0 text-left" width="50%">
                        {{ get_label('product_service', 'Product/Service') }}</th>
                    <th scope="col" class="border-0 pl-0 text-left">{{ get_label('quantity', 'Quantity') }}</th>
                    <th scope="col" class="border-0 pl-0 text-center" width="20%">{{ get_label('unit', 'Unit') }}
                    </th>
                    <th scope="col" class="border-0 pl-0 text-center" width="30%">{{ get_label('rate', 'Rate') }}
                    </th>
                    <th scope="col" class="border-0 pl-0 text-center" width="30%">{{ get_label('tax', 'Tax') }}
                    </th>
                    <th scope="col" class="border-0 pr-0 text-left" width="40%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="pl-0 text-center">{{ get_label('sub_total', 'Sub total') }}</td>
            ...
            <tr>
                <td colspan="5" class="border-1"></td>
                <td class="pl-0 text-center">{{ get_label('tax', 'Tax') }}</td>
                <td class="pr-0 text-center">
            ...
            <tr>
                <td colspan="5" class="border-1"></td>
                <td class="pl-0 text-center">{{ get_label('final_total', 'Final total') }}</td>
                <td class="total-amount pr-0 text-center">
    ...
    @if ($estimate_invoice->type == 'invoice')
        ...
        <table class="table-items table">
            <thead>
                <tr>
                    <th class="party-header border-0 pl-0" width="20%">
                ...
                @if (count($estimate_invoice->payments) > 0)
                    <tr>
                        ...
                        <th scope="col" class="border-1" width="30%">
                        ...
                        <th scope="col" class="border-1 pr-0" width="40%">
                            {{ get_label('amount_left', 'Amount left') }}</th>
                    </tr>
                @else
                    <tr>
                        <th class="border-0 pl-0">
                            {{ get_label('no_payments_found_invoice', 'No payments found for this invoice.') }}</th>
                    </tr>
                @endif
            </thead>
            <tbody>
