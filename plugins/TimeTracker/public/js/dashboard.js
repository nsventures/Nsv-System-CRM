$(function () {
    ...
    function renderDailyBreakdownChart(dailyData) {
        ...
        let manualProcessingTimes = dailyData.map(item => (item.manual_processing_time || 0) / 60);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let workTimes = dailyBreakdown.map(item => {
                    let total = (item.active_time + item.manual_time + item.break_time + item.idle_time + (item.manual_processing_time || 0)) / 60;
                    return total;
                });
